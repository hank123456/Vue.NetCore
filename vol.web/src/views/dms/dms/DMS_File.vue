 ...
 *Author：jxx
 *Contact：xx
 *业务请在@/extension/dms/dms/DMS_File.jsx或DMS_File.vue文件编写
...
<template>
    ...
    >
        ...
        <div class="btn-box">
            <div v-if="downloadRow" style="text-align: center; margin-bottom: 12px; color: #606266;">
                <span>即将下载: {{ downloadRow.FileName }}.{{ downloadRow.Extension }}</span>
            </div>
            <el-button
                type="primary"
                class="full-btn"
...
<script setup lang="jsx">
    import extend from "@/extension/dms/dms/DMS_File.jsx";
    import viewOptions from './DMS_File/options.js'
        ...
        switch (mode) {
            case 'original':
                ...
                try {
                    ...
                    const downloadResult = await proxy.http.post('/api/DMS_File/download', downloadData,true);
        ...
        if(gridRef.currentAction === 'Add'){
            ...
            if (gridRef.selectedFiles && gridRef.selectedFiles.length > 0) {
                ...
                try {
                    ...
                    // 上传文件到服务器
                    const uploadResult = await proxy.http.post('/api/DMS_File/upload', uploadFormData,true, {
                        headers: {
